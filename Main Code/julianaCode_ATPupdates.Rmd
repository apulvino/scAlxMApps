...
nichols_dr_seurat <- readRDS("/Users/anthonypulvino/NicholsLabwork/scAlxMApps/nichols_dr_seurat_2019.rds") #whatever the location of the file download
...
# Plot the clusters
DimPlot(object = nichols_umap_clusters, reduction.use = "umap", 
        do.return = TRUE,
        pt.size = 1.3)
...
## FIGURE 1 MARKERS FOR PAPER
nichols_umap.markers1 <- FindAllMarkers(nichols_umap_clusters, only.pos = TRUE, min.pct = 0.1, 
                                       thresh.use = 0.25)

nichols_umap.markers4 <- FindAllMarkers(nichols_umap_clusters, only.pos = TRUE, min.pct = 0.1, 
                                       thresh.use = 0.20)
nichols_umap.markers1 %>% group_by(cluster) %>% top_n(2, avg_logFC)

#nichols_umap.markers <- FindAllMarkers(nichols_umap_clusters, min.pct = 0.1, 
#                                       thresh.use = 0.25)

#nichols_umap.markers <- FindAllMarkers(nichols_umap_clusters, min.pct = 0.1, thresh.use = 0.25)

frontonasal.de.markers1 <- subset(nichols_umap.markers1, nichols_umap.markers1$cluster=="Frontonasal")
frontonasal.de.markers1 <- frontonasal.de.markers1[order(frontonasal.de.markers1$pct.2, -frontonasal.de.markers1$pct.1),]
View(frontonasal.de.markers1)
write.csv(frontonasal.de.markers1, "/Users/anthonypulvino/frontlist.csv")

arches.de.markers1 <- subset(nichols_umap.markers1, nichols_umap.markers1$cluster=="Arches 1,2")
arches.de.markers1 <- arches.de.markers1[order(arches.de.markers1$pct.2, -arches.de.markers1$pct.1),]
View(arches.de.markers1)
write.csv(arches.de.markers1, "/Users/anthonypulvino/a12list.csv")


posterior.de.markers1 <- subset(nichols_umap.markers1, nichols_umap.markers1$cluster=="Posterior")
posterior.de.markers1 <- posterior.de.markers1[order(posterior.de.markers1$pct.2, -posterior.de.markers1$pct.1),]
View(posterior.de.markers1)
write.csv(posterior.de.markers1, "/Users/anthonypulvino/postlist.csv")

melanocyte.de.markers1 <- subset(nichols_umap.markers1, nichols_umap.markers1$cluster=="Melanocyte")
melanocyte.de.markers1 <- melanocyte.de.markers1[order(melanocyte.de.markers1$pct.2, -melanocyte.de.markers1$pct.1),]
View(melanocyte.de.markers1)
write.csv(melanocyte.de.markers1, "/Users/anthonypulvino/melanolist.csv")
...
nichols_TSNE <- RunTSNE(nichols_dr_seurat, dims.use = 1:10, do.fast= TRUE, seed.use = 123)
nichols_TSNE <- SetAllIdent(nichols_TSNE, 'orig.ident')
...
#View(nichols_TSNE)
# Run the clustering. Change resolution to get more clusters by adding 0.1 and running again.
nichols_umap_clusters <- FindClusters(object = nichols_TSNE, reduction.type = "pca", dims.use = 1:10, resolution = 0.2, save.SNN = TRUE, n.start = 10, nn.eps = 0.5, print.output = FALSE, force.recalc = TRUE)
...
nichols_umap.markers <- FindAllMarkers(nichols_umap_clusters, only.pos = TRUE, min.pct = 0.25, 
                                       thresh.use = 0.25, min.diff.pct = .1)
...
nichols_umap.markers <- FindAllMarkers(nichols_UMAP, min.pct = 0.1, 
                                       thresh.use = 0.25)
nichols_umap.markers <- FindAllMarkers(nichols_UMAP, min.pct = 0.1, 
                                       thresh.use = 0.25, min.diff.pct = .1)
nichols_umap.markers <- FindAllMarkers(nichols_UMAP, min.pct = 0.25, 
                                       thresh.use = 0.25, min.diff.pct = .1)
#nichols_umap.markers <- FindAllMarkers(nichols_umap_clusters, min.pct = 0.25, 
#                                       thresh.use = 0.25, min.diff.pct = .1)
nichols_umap.markers %>% group_by(cluster) %>% top_n(2, avg_logFC)
View(nichols_umap.markers)
...
nichols.markers <- FindAllMarkers(nichols_umap_clusters, min.pct = 0.25, logfc.threshold = 0.25)
nichols.markers %>% group_by(cluster) %>% top_n(n = 50, wt = avg_logFC)
View(nichols.markers)

nichols.markers <- FindMarkers(nichols_umap_clusters, ident.1 = "Frontonasal", ident.2 = NULL, min.pct = 0.1, logfc.threshold = 0.25)
...
ARCHESmarkers <- subset(nichols.markers, nichols.markers$cluster=="Arches 1,2")
FNmarkers <- subset(nichols.markers, nichols.markers$cluster=="Frontonasal")
View(ARCHESmarkers)
#To revert to old Seurat v2.3.4
source("https://z.umn.edu/archived-seurat")

## USED TO GENERATE GENE LISTS FOR PAPER SUPP FIGURE GENE LISTS --ATP
...
melanocyte.de.markers <- subset(nichols.markers, nichols.markers$cluster=="*")
...
FNgene_names <- frontonasal.de.markers[order(frontonasal.de.markers$pct.2, -frontonasal.de.markers$pct.1),][,7]
ARgene_names <- arches.de.markers[order(arches.de.markers$pct.2, -arches.de.markers$pct.1),][,7]
POSTgene_names <- posterior.de.markers[order(posterior.de.markers$pct.2, -posterior.de.markers$pct.1),][,7]
STARgene_names <- melanocyte.de.markers[order(melanocyte.de.markers$pct.2, -melanocyte.de.markers$pct.1),][,7]

featureplots_FN <- FeaturePlot(nichols_umap_clusters, features.plot = FNgene_names[1:15], reduction.use = "umap", cols.use = c("grey","blue"), no.axes = FALSE, no.legend = FALSE, dark.theme = FALSE, do.return = TRUE, max.cutoff = 1.25)  + theme(panel.grid = element_rect())+ xlim (c(-10, 10))  

featureplots_AR <- FeaturePlot(nichols_umap_clusters, features.plot = ARgene_names[1:15], reduction.use = "umap", cols.use = c("grey","blue"), no.axes = FALSE, no.legend = FALSE, dark.theme = FALSE, do.return = TRUE, max.cutoff = 1.25)  + theme(panel.grid = element_rect())+ xlim (c(-10, 10))  

featureplots_POST <- FeaturePlot(nichols_umap_clusters, features.plot = POSTgene_names[1:15], reduction.use = "umap", cols.use = c("grey","blue"), no.axes = FALSE, no.legend = FALSE, dark.theme = FALSE, do.return = TRUE, max.cutoff = 1.25)  + theme(panel.grid = element_rect())+ xlim (c(-10, 10))  

featureplots_STAR <- FeaturePlot(nichols_umap_clusters, features.plot = STARgene_names[1:15], reduction.use = "umap", cols.use = c("grey","blue"), no.axes = FALSE, no.legend = FALSE, dark.theme = FALSE, do.return = TRUE, max.cutoff = 1.25)  + theme(panel.grid = element_rect())+ xlim (c(-10, 10))  
```
